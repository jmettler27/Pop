...
export function ReactHookFormTextInput({ label, maxLength, validationSchema, ...props }: FieldProps & {
  maxLength?: number;
  type?: string;
  placeholder?: string;
}) {
  ...
  return (
    <>
      <label className="mt-4 block text-[clamp(0.875rem,1.2vw,1rem)] font-medium" htmlFor={props.id || field.name}>
        ...
        {label}{' '}
        {maxLength ? numCharsIndicator(field.value || '', maxLength) : null}
      </label>
      <Input
        {...field}
        {...props}
...
export function ReactHookFormSelect({ label, validationSchema, children, ...props }: FieldProps & {
  ...
  return (
    <>
      ...
      >
        <SelectTrigger id={props.id || field.name} onBlur={field.onBlur} className="w-full max-w-[400px] cursor-pointer">
          <SelectValue />
        </SelectTrigger>
