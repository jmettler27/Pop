        ...
        >
            <FieldArray name='choices'>
                <Box component='section' sx={{ my: 2, p: 2, border: '2px dashed grey', width: '500px' }}>
                    {values.choices.map((_item, index) => (
                        <div key={index}>
                            <label htmlFor={`choices.${index}`}>{NaguiQuestion.CHOICES[index]} ({values.choices[index].length}/{NaguiQuestion.CHOICE_MAX_LENGTH})</label>
                            <Field
                                name={`choices.${index}`}
                                type='text'
            ...
            >
                ...
                {values.choices.map((choice, index) => (
                    <option key={index} value={index}>{NaguiQuestion.CHOICES[index]}. {choice}</option>
                ))}
            </MySelect>
