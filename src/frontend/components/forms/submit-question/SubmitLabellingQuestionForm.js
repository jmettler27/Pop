...
import { ADD_ITEM, QUESTION_HINTS_REMARKS, QUESTION_ITEM, QUESTION_TITLE_LABEL, SUBMIT_QUESTION_BUTTON_LABEL } from '@/frontend/utils/forms/questions';
...
function EnterLabels({ validationSchema, lang }) {
    ...
    return (
        <>
            <p>{NUM_LABELS_ALLOWED[lang]}: {LabellingQuestion.MIN_NUM_LABELS}-{LabellingQuestion.MAX_NUM_LABELS}.</p>
                        ...
                        {values.labels.length > 0 &&
                            values.labels.map((item, index) => (
                                <Box key={index} component='section' sx={{ my: 2, pb: 2, px: 2, border: '2px dashed grey', width: '500px' }}>
                                    <label htmlFor={'labels.' + index}>{requiredStringInArrayFieldIndicator(validationSchema, 'labels')}{QUESTION_ITEM[lang]} #{index + 1} {numCharsIndicator(item, LabellingQuestion.LABEL_MAX_LENGTH)}</label>
                                    <Field
                                        name={'labels.' + index}
                                        type='text'
