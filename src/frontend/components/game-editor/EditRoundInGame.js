    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        <CardTitle className="2xl:text-2xl">
          {roundTypeToEmoji(round.type)} <i>{round.title}</i> <RoundTopicDistribution round={round} />
        </CardTitle>
  ...
  return (
    <span className="2xl:text-xl">
      ...
      {Object.entries(topics)
        .map(([topic, count]) => `${topicToEmoji(topic)}x${count}`)
        .join(', ')}
      )
    </span>
  );
...
function EditGameRoundQuestionCards({ round, status }) {
  return round.questions.map((questionId, idx) => (
    <EditQuestionCard key={questionId} roundId={round.id} questionId={questionId} questionOrder={idx} status={status} />
  ));
}
