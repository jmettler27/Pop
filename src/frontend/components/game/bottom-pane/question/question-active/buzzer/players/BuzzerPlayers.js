    ...
    return (
        <ol className='overflow-auto'>
            {buzzed.map((playerId, index) => (
                <li key={index} className='2xl:text-xl'>{rankingToEmoji(index)} {getPlayerName(gamePlayers, playerId)}</li>
            ))}
        </ol>
    )
}
    ...
    return (
        <ol className='overflow-auto'>
            {canceled.map((item, index) => (
                <li key={index} className='2xl:text-xl'>💩 {getPlayerName(gamePlayers, item.playerId)} {item.clueIdx >= 0 && `(#${item.clueIdx + 1})`}</li>
            ))}
        </ol>
    )
}