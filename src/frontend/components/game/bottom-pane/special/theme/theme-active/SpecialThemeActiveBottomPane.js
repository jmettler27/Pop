...
export default function SpecialThemeActiveBottomPane({ theme, gameTheme }) {
    ...
    switch (myRole) {
        ...
        default:
            return <span className='2xl:text-4xl font-bold'><GameChooserHelperText chooserTeamId={gameTheme.teamId} /></span>
    }
...
function SpecialThemeActiveOrganizerBottomPane({ theme, gameTheme }) {
    ...
    const gameSectionRef = doc(GAMES_COLLECTION_REF, game.id, 'rounds', game.currentRound, 'themes', theme.id, 'sections', currentSectionId)
...
function SpecialQuestionEndOrganizerBottomPane({ theme, gameTheme, gameSection }) {
    ...
    const [handleContinue, isHandling] = useAsyncAction(async () => {
        await handleQuestionEndOrganizerContinue(game.id, game.currentRound, theme.id, gameSection.id, isLastQuestionInSection, isLastSectionInTheme, user.id)
    })
