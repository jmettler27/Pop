...
function ProgressTabPanelMainContent({ game, lang = DEFAULT_LOCALE }) {
    ...
    return (
        <Box className='w-full'>
            ...
            {(game.type === 'rounds' && game.currentRound && (game.status === GameStatus.ROUND_START || game.status === GameStatus.ROUND_END || game.status === GameStatus.QUESTION_ACTIVE || game.status === GameStatus.QUESTION_END || game.status === GameStatus.SPECIAL)) && (
