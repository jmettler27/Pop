...
function SpecialRoundHomeProgress({ round, lang = DEFAULT_LOCALE }) {
    ...
    const [gameThemes, gameThemesLoading, gameThemesError] = useCollection(query(gameThemesCollectionRef, where('order', '!=', null), orderBy('order', 'asc')))

    const [teams, teamsLoading, teamsError] = useCollectionOnce(collection(GAMES_COLLECTION_REF, gameId, 'teams'))

    if (gameThemesError) {
        return <p><strong>Error: {JSON.stringify(gameThemesError)}</strong></p>
    }
    if (teamsError) {
                    ...
                    >
                        ...
                        >
                            <Typography className={clsx(
                                ...
                                <span className='font-bold'>{THEME_TEXT[lang]} {gameTheme.order + 1}</span>: {gameTheme.title}
                            </Typography>
                        </AccordionSummary>
                        <AccordionDetails>
                            <Typography>
...
function SpecialRoundThemeProgress({ round, lang = DEFAULT_LOCALE }) {
    ...
    useEffect(() => {
        if (!gameTheme || gameTheme.currentSectionIdx === null)
            return
    ...
    const sortedGameSections = theme.details.sections.map((sectionId) => gameSections.docs.find(doc => doc.id === sectionId)).map((doc) => ({ id: doc.id, ...doc.data() }))
    ...
    return (
        <>
            <h2 className='text-lg'>{topicToEmoji(theme.topic)} <strong>{THEME_TEXT[lang]} {gameTheme.order + 1}</strong>: {theme.details.title}</h2>

            <div className='w-full m-1 px-2'>
                {sortedGameSections.map((gameSection, idx) => (
                    <ThemeSectionAccordion key={gameSection.id}
                        themeId={themeId}
...
function ThemeSectionAccordion({ themeId, sectionId, sectionOrder, hasEnded, isCurrent, hasNotStarted, onAccordionChange, expanded, gameSection, currentGameSection, playerTeam, lang = DEFAULT_LOCALE }) {
    ...
    return (
        <ol className='list-decimal list-inside'>
            {gameSection.question_status.map((status, idx) => {
                ...
                const questionHasEnded = sectionHasEnded || (sectionIsCurrent && idx < currentQuestionIdx) || (questionisCurrent && status === GameStatus.QUESTION_END)
