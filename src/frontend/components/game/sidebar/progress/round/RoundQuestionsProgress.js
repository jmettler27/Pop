...
export const RoundQuestionAccordion = memo(function RoundQuestionAccordion({ game, roundId, roundType, questionId, order, hasEnded, isCurrent, hasNotStarted, onAccordionChange, expanded, teams, players }) {
    ...
    const showComplete = myRole === UserRole.ORGANIZER || (isCurrent && game.status === GameStatus.QUESTION_END) || hasEnded || (game.status === GameStatus.ROUND_END)
...
function RoundQuestionSummary({ roundType, question, order, lang = DEFAULT_LOCALE }) {
    ...
    switch (question.type) {
        ...
        case QuestionType.BLINDTEST:
            return <span className='text-lg'>{BlindtestQuestion.typeToEmoji(question.details.subtype)}{topicToEmoji(question.topic)} <strong>{questionTypeToTitle(question.type, lang)} {order + 1}</strong></span>
