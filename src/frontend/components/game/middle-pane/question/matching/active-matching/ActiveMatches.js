...
import { MatchingNode, MatchingEdge, getNode, getNodeText, matchIsComplete } from '@/frontend/components/game/middle-pane/question/matching/gridUtils';
...
function ActiveMatchingQuestionNodes({ answer, nodePositions, numCols, edges, setEdges, newEdgeSource, setNewEdgeSource, deselectOnNewEdge = true }) {
    ...
    const { isCanceled, loading: isCanceledLoading, error: isCanceledError } = gameQuestionRepo.useIsCanceled(game.currentQuestion, myTeam)
    const { correctMatches, loading: correctMatchesLoading, error:  correctMatchesError } = gameQuestionRepo.useCorrectMatches(game.currentQuestion)

    if (isChooserError) {
        return <p><strong>Error: {JSON.stringify(isChooserError)}</strong></p>
    }
    if (isCanceledError) {
        return <p><strong>Error: {JSON.stringify(isCanceledError)}</strong></p>
    }
    if (correctMatchesError) {
        return <p><strong>Error: {JSON.stringify(correctMatchesError)}</strong></p>
    }
