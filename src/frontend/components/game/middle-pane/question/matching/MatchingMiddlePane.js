...
function MatchingQuestionGrid({ baseQuestion }) {
    ...
    return <>
        {game.status === GameStatus.QUESTION_ACTIVE && <ActiveMatchingQuestionGrid answer={answer} nodePositions={nodePositions} numCols={numCols} />}
        {game.status === GameStatus.QUESTION_END && <EndedMatchingQuestionGrid answer={answer} nodePositions={nodePositions} numCols={numCols} numRows={numRows} />}
    </>
}