    ...
    return (
        <div className='flex flex-col items-center justify-center space-y-2'>
            <div className='flex flex-row items-center justify-center space-x-1'>
                ...
                <h1 className='2xl:text-5xl'>{topicToEmoji(baseQuestion.topic)} <strong>{questionTypeToTitle(baseQuestion.type)} <CurrentRoundQuestionOrder /></strong></h1>
            </div>
            <div className='flex flex-row items-center justify-center space-x-1'>
...
function OddOneOutMainContent({ baseQuestion, randomization }) {
    ...
    return <OddOneOutProposals baseQuestion={baseQuestion} randomization={randomization} selectedItems={gameQuestion.selectedItems} isChooser={isChooser} authorized={timer.authorized} />
}
...
function ProposalItem({ item, onProposalClick, onAccordionChange, selectedItem, expanded, isOdd, isLast, isChooser, authorized, isSubmitting }) {
    ...
    const isItemInteractive = myRole === UserRole.ORGANIZER || (myRole === UserRole.PLAYER && isChooser && authorized && !showExplanation);
...
function SelectedProposalPlayerAvatar({ playerId }) {
    ...
    return !playerError && !playerLoading && player && (
        <Avatar
            alt={player.name}
            src={player.image}
