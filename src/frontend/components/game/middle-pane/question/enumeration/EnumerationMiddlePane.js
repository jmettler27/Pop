...
function EnumerationQuestionObjective({ question, lang = DEFAULT_LOCALE }) {
    switch (lang) {
        case 'en':
            return <span className='2xl:text-3xl text-yellow-300'>There are {question.maxIsKnown ? "exactly" : "at least"} <strong>{question.answer.length}</strong> answers</span>
        case 'fr-FR':
            return <span className='2xl:text-3xl text-yellow-300'>Il y a {question.maxIsKnown ? "exactement" : "au moins"} <strong>{question.answer.length}</strong> réponses</span>
    }
}
    ...
    return (
        <ul className='list-disc pl-10 h-full w-full flex flex-col flex-wrap overflow-auto items-center justify-center'>
            {answer.map((item, index) => {
                ...
                const isSelectable = !isSubmitting && (
