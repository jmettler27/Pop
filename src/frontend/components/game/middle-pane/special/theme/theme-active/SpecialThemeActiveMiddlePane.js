...
function SectionTitle({ section, gameTheme, lang = DEFAULT_LOCALE }) {
    return <h1 className='2xl:text-4xl'><span className='font-bold'>{THEME_SECTION_TEXT[lang]} {gameTheme.currentSectionIdx + 1}</span>{section.title && `: ${section.title}`}</h1>
}
...
function SectionQuestions({ currentThemeId, currentSectionId, sectionQuestions }) {
    ...
    const gameSectionRef = doc(GAMES_COLLECTION_REF, game.id, 'rounds', game.currentRound, 'themes', currentThemeId, 'sections', currentSectionId)
    const [gameSection, gameSectionLoading, gameSectionError] = useDocumentData(gameSectionRef)
    if (gameSectionError) {
        return <p><strong>Error: {JSON.stringify(gameSectionError)}</strong></p>
    }
