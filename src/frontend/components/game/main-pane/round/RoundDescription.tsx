...
const correct = (chunks: ReactNode[]) => <span className="text-green-500">{keyChunks(chunks)}</span>;
const incorrect = (chunks: ReactNode[]) => <span className="font-bold text-red-500">{keyChunks(chunks)}</span>;
...
export function RoundDescription({ round }: { round: AnyRound }) {
  switch (round.type) {
