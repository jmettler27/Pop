...
function RoundRankingPolicyTitle({ round }: { round: AnyRound }) {
  switch (round.type) {
    ...
    case RoundType.NAGUI:
      return <BuzzerRoundRankingPolicyTitle round={round as BuzzerRound} />;
    case RoundType.LABELLING:
      return <LabellingRoundRankingPolicyTitle round={round as LabellingRound} />;
    case RoundType.MATCHING:
      return <MatchingRoundRankingPolicyTitle round={round as MatchingRound} />;
    case RoundType.MCQ:
      return <MCQRoundRankingPolicyTitle round={round as MCQRound} />;
    case RoundType.ODD_ONE_OUT:
      return <OddOneOutRoundRankingPolicyTitle round={round as OddOneOutRound} />;
    case RoundType.QUOTE:
      return <QuoteRoundRankingPolicyTitle round={round as QuoteRound} />;
    case RoundType.REORDERING:
      return <ReorderingRoundRankingPolicyTitle round={round as ReorderingRound} />;
    default:
      return <></>;
  }
}

function BuzzerRoundRankingPolicyTitle({ round }: { round: BuzzerRound }) {
  const { formatMessage } = useIntl();
  return (
...
function QuoteRoundRankingPolicyTitle({ round }: { round: QuoteRound }) {
  const { formatMessage } = useIntl();
  return (
    <h1 className="2xl:text-3xl">
...
function LabellingRoundRankingPolicyTitle({ round }: { round: LabellingRound }) {
  const { formatMessage } = useIntl();
  return (
    <h1 className="2xl:text-3xl">
      {fmt(formatMessage, messages.pointsPerElementFound, {
...
function OddOneOutRoundRankingPolicyTitle({ round }: { round: OddOneOutRound }) {
  const { formatMessage } = useIntl();
...
function MatchingRoundRankingPolicyTitle({ round }: { round: MatchingRound }) {
  const { formatMessage } = useIntl();
  const mistakePenalty = (round as unknown as { mistakePenalty: number }).mistakePenalty;
  return (
    <h1 className="2xl:text-3xl">
...
function MCQRoundRankingPolicyTitle({ round }: { round: MCQRound }) {
  const { formatMessage } = useIntl();
  return (
    <>
...
function ReorderingRoundRankingPolicyTitle({ round }: { round: ReorderingRound }) {
  const { formatMessage } = useIntl();
  return (
    <h1 className="2xl:text-3xl">
