...
for (const file of walkFiles(SRC_DIR)) {
  ...
  while ((openMatch = OPEN_RE.exec(src)) !== null) {
    ...
    while ((entry = ENTRY_RE.exec(body)) !== null) {
      const key = entry[1];
      const value = (entry[2] ?? entry[3]).replace(/\\'/g, "'").replace(/\\"/g, '"').replace(/\\\\/g, '\\');
...
if (missingInFr.length)
  console.warn(
    `⚠ Added ${missingInFr.length} missing keys with empty value (needs translation):\n  ${missingInFr.join('\n  ')}`
  );
if (staleInFr.length) console.warn(`⚠ Stale fr keys not in source (${staleInFr.length}):\n  ${staleInFr.join('\n  ')}`);
if (!missingInFr.length && !staleInFr.length) console.log('✓ Catalogs are in sync — no missing or stale keys');
