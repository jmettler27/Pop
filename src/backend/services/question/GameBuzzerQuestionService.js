...
export default class GameBuzzerQuestionService extends GameQuestionService  {
    ...
    async resetQuestion(questionId) {
        ...
        try {
            await runTransaction(firestore, async (transaction) =>
                this.resetQuestionTransaction(transaction, questionId));
        }
        catch (error) {
    ...
    async handleCountdownEndTransaction(transaction, questionId) {
        ...
        if (buzzed.length === 0)
            ...
            await invalidateBuzzerAnswerTransaction(transaction, gameId, roundId, questionId, buzzed[0], questionType)
