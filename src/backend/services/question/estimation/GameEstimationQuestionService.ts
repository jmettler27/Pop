...
export default class GameEstimationQuestionService extends GameQuestionService {
  ...
  async resetQuestionTransaction(transaction: Transaction, questionId: string) {
    const gameQuestion = (await (this.gameQuestionRepo as GameEstimationQuestionRepository).getQuestionTransaction(
      transaction,
      questionId
  ...
  async calculateRewardsAndProgressTransaction(
    ...
    await (this.gameQuestionRepo as GameEstimationQuestionRepository).updateQuestionWinnersTransaction(
      transaction,
      questionId,
      Array.from(winnerTeamIds)
    );
  ...
  async submitBetTransaction(
    ...
    if (bets.length >= numTeams) {
      ...
      await (this.gameQuestionRepo as GameEstimationQuestionRepository).updateTransaction(transaction, questionId, {
        bets,
      });
      await super.endQuestionTransaction(transaction, questionId);
    } else {
