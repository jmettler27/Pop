...
export default class RoundService {
    ...
    constructor(gameId, roundId, roundType) {
        if (!gameId) {
    ...
    async switchRoundQuestionTransaction(transaction, questionOrder) {
        ...
        await this.prepareQuestionStartTransaction(transaction, questionId, questionOrder)
    
        await this.roundRepo.updateRoundTransaction(transaction, this.roundId, {
    ...
    async handleRoundSelected(roundId, userId) {
        ...
        try {
            await runTransaction(firestore, transaction => handleRoundSelectedTransaction(transaction, roundId, userId))
        } catch (error) {
    ...
    async handleRoundSelectedTransaction(transaction, roundId, userId) {    
        ...
        console.log('Round successfully started', 'roundId', this.roundId)
    }
    ...
    async startRoundTransaction(transaction) {
        await this.switchRoundQuestionTransaction(transaction, this.roundId, 0)
     ...
     */
    async handleQuestionEnd(roundId) {
        if (!roundId) {
            throw new Error("Round ID is required");
        }

        try {
            await runTransaction(firestore, transaction => handleQuestionEndTransaction(transaction, roundId))
        } catch (error) {
            console.error("Error handling question end:", error);
            throw error;
        }
    }
    async handleQuestionEndTransaction(transaction) {
        ...
        await (round.isLastQuestion() ?
            ...
            this.switchRoundQuestionTransaction(transaction, round.currentQuestionIdx + 1) /* Prepare the next question */
        )
    }
    ...
    async endRound(roundId) {
        ...
        try {
            await runTransaction(firestore, transaction => endRoundTransaction(transaction, roundId))
        } catch (error) {
    ...
    async endRoundTransaction(transaction, roundId) {
        ...
        const roundScoresData = await this.roundScoreRepo.getScores(this.roundId)
            ...
            if (![RoundType.ODD_ONE_OUT, RoundType.MATCHING, RoundType.SPECIAL].includes(roundType)) {
                ...
                if ([RoundType.MCQ, RoundType.NAGUI].includes(roundType)) {
                    ...
                    roundCompletionRates = Object.entries(teamStats).reduce((acc, [teamId, { sumRewards, numQuestions }]) => {
                        const maxPoints = roundType === RoundType.NAGUI
        ...
        if (round.order > 0) {
            const prevRound = await this.roundRepo.getRoundsByQueryTransaction({
                where: {
                    field: 'order',
                    operator: '==',
                    value: round.order - 1
        ...
        if (round.order < gameData.rounds.length - 1) {
            ...
            for (const [idx, teamId] of newChooserOrder.entries()) {
                ...
                for (const playerDoc of playersSnapshot.docs) {
                    await this.playerRepo.updatePlayerStatusTransaction(transaction, playerDoc.id, idx === 0 ? PlayerStatus.FOCUS : PlayerStatus.IDLE)
                }
            }
            await this.chooserRepo.updateChooserTransaction(transaction, {
                chooserIdx: 0,
