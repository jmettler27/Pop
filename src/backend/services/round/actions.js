...
export async function handleQuestionEnd(gameId, roundId) {
    ...
    try {
        await runTransaction(firestore, async (transaction) =>
            handleQuestionEndTransaction(transaction, gameId, roundId)
        )
    } catch (error) {
...
const handleQuestionEndTransaction = async (
    transaction,
    gameId,
    roundId,
) => {
...
const handleRoundSelectedTransaction = async (
    transaction,
    gameId,
    roundId,
    userId
...
const moveToNextQuestionTransaction = async (
    ...
    if (baseQuestion.type === QuestionType.MCQ || baseQuestion.type === QuestionType.NAGUI) {
        ...
        if (baseQuestion.type === QuestionType.MATCHING) {
            ...
            await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, duration: defaultThinkingTime * (baseQuestion.details.numCols - 1) })
        } else {
            // await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, managedBy, duration: defaultThinkingTime })
            await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, duration: defaultThinkingTime })
        }
        ...
        if (baseQuestion.type === QuestionType.ENUMERATION) {
            ...
            await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, duration: baseQuestion.details.thinkingTime })
        }
        else {
            // await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, managedBy, duration: defaultThinkingTime })
            await updateTimerTransaction(transaction, gameId, { status: TimerStatus.RESET, duration: defaultThinkingTime })
