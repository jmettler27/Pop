...
export default class RoundService {
  ...
  async endRoundTransaction(transaction: Transaction, roundId: string) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const gameScoresData = ((await this.gameScoreRepo.getScoresTransaction(transaction)) as any) ?? {};
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const roundScoresData = ((await roundScoreRepo.getScoresTransaction(transaction)) as any) ?? {};
