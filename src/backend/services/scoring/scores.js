...
import { collection, query, getDocs, doc, updateDoc, increment, runTransaction } from 'firebase/firestore';

import { getDocData, getDocDataTransaction } from '@/backend/services/utils';
...
const initGameScoresTransaction = async (transaction, gameId) => {
  const teamsCollectionRef = collection(GAMES_COLLECTION_REF, gameId, 'teams');
  const teamsSnapshot = await getDocs(query(teamsCollectionRef));
