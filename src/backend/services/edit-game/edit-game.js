...
const addRoundToGameTransaction = async (transaction, gameId, title, type) => {
  ...
  if (type === RoundType.MIXED) {
    ...
    initRoundInfo.mistakePenalty =
      roundScorePolicy === ScorePolicyType.COMPLETION_RATE ? -20 : OddOneOutRound.DEFAULT_MISTAKE_PENALTY;
  } else if (type === RoundType.MATCHING) {
    initRoundInfo.mistakePenalty =
      roundScorePolicy === ScorePolicyType.COMPLETION_RATE ? -5 : MatchingRound.DEFAULT_MISTAKE_PENALTY;
...
const addQuestionToRoundTransaction = async (transaction, gameId, roundId, questionId, managerId) => {
  ...
  if (isBuzzer(baseQuestion.type)) {
    ...
    const gameQuestionPlayersRef = doc(
...
const removeRoundFromGameTransaction = async (transaction, gameId, roundId) => {
  const roundRef = doc(GAMES_COLLECTION_REF, gameId, 'rounds', roundId);
...
const removeQuestionFromRoundTransaction = async (transaction, gameId, roundId, questionId, questionType = null) => {
  const type =
    questionType || (await getDocDataTransaction(transaction, doc(QUESTIONS_COLLECTION_REF, questionId))).type;
...
const updateQuestionCreatorTransaction = async (transaction, gameId, roundId, questionId, userId) => {
  const baseQuestionRef = doc(QUESTIONS_COLLECTION_REF, questionId);
...
const addGameOrganizerTransaction = async (transaction, gameId, organizerId) => {
...
const launchGameTransaction = async (transaction, gameId) => {
