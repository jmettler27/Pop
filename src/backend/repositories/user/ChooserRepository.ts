...
export default class ChooserRepository extends FirebaseDocumentRepository {
  ...
  async resetAndGetChoosersTransaction(transaction: Transaction): Promise<Chooser | null> {
    const current = await this.getTransaction(transaction);
    if (!current) return null;
    await this.updateTransaction(transaction, { chooserIdx: 0 });
    return { ...current, chooserIdx: 0 } as unknown as Chooser;
  }
