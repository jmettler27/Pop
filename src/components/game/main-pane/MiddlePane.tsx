import GameEndMiddlePane from '@/components/game/main-pane/game/GameEndMiddlePane';
import GameHomeMiddlePane from '@/components/game/main-pane/game/GameHomeMiddlePane';
import GameStartMiddlePane from '@/components/game/main-pane/game/GameStartMiddlePane';
import QuestionMiddlePane from '@/components/game/main-pane/question/QuestionMiddlePane';
import RoundMiddlePane from '@/components/game/main-pane/round/RoundMiddlePane';
import useGame from '@/hooks/useGame';
import { GameStatus } from '@/models/games/game-status';

export default function MiddlePane() {
  const game = useGame();
  if (!game) return null;

  switch (game.status) {
    case GameStatus.GAME_START:
      return <GameStartMiddlePane />;

    case GameStatus.GAME_HOME:
      return <GameHomeMiddlePane />;

    case GameStatus.ROUND_START:
    case GameStatus.ROUND_END:
      return <RoundMiddlePane />;

    case GameStatus.QUESTION_ACTIVE:
    case GameStatus.QUESTION_END:
      return <QuestionMiddlePane />;

    case GameStatus.GAME_END:
      return <GameEndMiddlePane />;

    default:
      return <></>;
  }
}
