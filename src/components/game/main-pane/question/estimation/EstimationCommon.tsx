...
import { formatDuration, timestampElapsedSeconds, type FirestoreTimestamp } from '@/helpers/time';
import { useAllPlayersOnce } from '@/hooks/firestore/user/usePlayerHooks';
...
function BetResultRow({ team, player, bet, isWinner, baseQuestion, gameQuestion }: BetResultRowProps) {
  ...
  const betDisplay = bet
    ? bet.type === EstimationQuestion.BetType.EXACT
      ...
      : `${formatAnswerValue(baseQuestion.answerType, (bet as RangeEstimationBet).lower, intl.locale)} – ${formatAnswerValue(baseQuestion.answerType, (bet as RangeEstimationBet).upper, intl.locale)}`
    : null;
