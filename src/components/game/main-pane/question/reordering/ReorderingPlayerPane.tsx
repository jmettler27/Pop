...
} from '@/components/game/main-pane/question/reordering/ReorderingCommon';
...
function ReorderingPlayerActiveView({ baseQuestion, gameQuestion, randomMapping }: ReorderingPlayerActiveViewProps) {
  ...
  const [handleSubmitOrdering, isSubmitting] = useAsyncAction(async () => {
    ...
    await questionAction(gameId, roundId, questionId, { action: QUESTION_ACTIONS.ReorderingSubmit, orderedTitles });
    setDialogOpen(false);
  });
