...
import { MySelect, MyTextInput, StyledErrorMessage } from '@/components/common/StyledFormComponents';
...
export default function SubmitMCQForm(props: QuestionFormProps) {
  ...
  const [submitMCQ, isSubmitting] = useAsyncAction(async (values: MCQFormValues) => {
    try {
      const { topic, lang, ...rest } = values as typeof values & { topic: Topic; lang: Locale };
      const data: QuestionFormPayload = { type: QUESTION_TYPE, topic, lang, details: rest };
