...
import { GAME_TYPE_TO_EMOJI } from '@/lib/utils/game'
...
import { RoundScoresCard } from './RoundScoresCard';
    ...
    return (
        <div className='h-screen flex flex-row divide-x divide-solid'>
            ...
            <div className='flex flex-col h-full w-5/6'>
                ...
                {/* Main content */}
                <GameAnalyticsDashboard game={game} />
            </div>
        </div>
    );
...
function GameAnalyticsDashboard({ game }) {
    console.log("RENDERING GameAnalyticsDashboard")


    return (
        <main className='flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6'>
            {game.status === 'game_end' && <GameRoundsCard game={game} />}
        </main>
    )
}
...
function GameRoundsCard({ game }) {
    ...
    const roundsRef = collection(GAMES_COLLECTION_REF, game.id, 'rounds')
    const roundsQuery = query(roundsRef, where('order', '!=', null), orderBy('order', 'asc'))
    ...
    if (teamsLoading) {
        return <LoadingScreen loadingText="Loading teams..." />
    }
    if (roundsLoading) {
        return <LoadingScreen loadingText="Loading rounds..." />
    }
    if (!teamsCollection || !roundsCollection) {
        return <></>
    }
    ...
    return (
        ...
        <Card>
            <CardHeader className='flex flex-row items-center justify-between pb-2 space-y-0'>
                <CardTitle className='2xl:text-2xl'>ROUNDS</CardTitle>
            </CardHeader>
            <CardContent>
                <div className='grid gap-4 md:grid-cols-2'>
                    {rounds.map(round => <RoundScoresCard key={round.id} round={round} teams={teams} />)}
                </div>
            </CardContent>
        </Card>
    )
