...
export default function GameScoreboard({ roundScores, teams, lang = 'en' }) {
...
import './RankDifferenceIcon.css'; // Import your custom CSS file
...
function RankDifferenceIcon({ rankDiff }) {
    ...
    return (
        <div className='rank-difference-icon-container'>
            {icon}
            {textContent && <div className={clsx(
                rankDiff > 0 ? 'rank-difference-text-up' : 'rank-difference-text-down'
            )}>{textContent}</div>}
        </div>
    );
}